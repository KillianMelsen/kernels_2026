...
if (!("results_BRIWECS_LOF_SE.rds" %in% list.files("BRIWECS_LOF"))) {
  # ADD ====
...
results2$Management <- factor(as.character(results2$Management), levels = c("High Nitrogen", "Low Nitrogen"), labels = c("High Nitrogen", "Low Nitrogen"))
results3$Management <- factor(as.character(results3$Management), levels = c("High Nitrogen", "Low Nitrogen"), labels = c("High Nitrogen", "Low Nitrogen"))
...
var
var / perc
ggsave(filename = "plots/BRIWECS_LOF_Averaged_numeric_combined_SE_extendedbaseline.png", dpi = 300, width = 32, height = 20, units = "cm")
...
ggplot(droplevels(results2[results2$Environment %in% levels(results2$Environment)[1:7],]), aes(fill = Component, y = Variance, x = Model)) +
  ...
  theme_classic(base_size = 18) + theme(legend.position = "bottom",
                                        ...
                                        axis.title.y = element_text(size = 20),
                                        axis.text.x = element_text(angle = 60, vjust = 1, hjust = 1.1))
ggsave(filename = "plots/BRIWECS_LOF_perEnv_numeric_SE_A_extendedbaseline.png", dpi = 300, width = 32, height = 48, units = "cm")
...
ggplot(droplevels(results2[results2$Environment %in% levels(results2$Environment)[8:14],]), aes(fill = Component, y = Variance, x = Model)) +
  ...
  theme_classic(base_size = 18) + theme(legend.position = "bottom",
                                        ...
                                        axis.title.y = element_text(size = 20),
                                        axis.text.x = element_text(angle = 60, vjust = 1, hjust = 1.1))
ggsave(filename = "plots/BRIWECS_LOF_perEnv_numeric_SE_B_extendedbaseline.png", dpi = 300, width = 32, height = 48, units = "cm")
