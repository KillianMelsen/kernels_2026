...
cvsets <- readRDS(sprintf("DROPS/results/CV2/cvsets.CV2.SE.%d-%d.rds", datasets[1], datasets[length(datasets)]))
...
for (nc in n.checks) {
  for (run in datasets) {
    ...
    try({
      ...
      bandwidths[bandwidths.row, c("R", "W", "R_W")] <- summary(mod.svar.GK)$varcomp$component[4:6]
    })
    toc(); tic.clearlog()
    ...
    try({
      ...
      bandwidths[bandwidths.row, c("R", "W", "R_W")] <- summary(mod.mvar.GK)$varcomp$component[30:32]
    })
    toc(); tic.clearlog()
  }
}
...
saveRDS(results, sprintf("DROPS/results/CV2/results.MB.CV2.SE.%d-%d.rds", datasets[1], datasets[length(datasets)]))
saveRDS(comptimes, sprintf("DROPS/results/CV2/comptimes.MB.CV2.SE.%d-%d.rds", datasets[1], datasets[length(datasets)]))
saveRDS(bandwidths, sprintf("DROPS/results/CV2/bandwidths.MB.CV2.SE.%d-%d.rds", datasets[1], datasets[length(datasets)]))
if (save.models) {
  saveRDS(mod.fits, sprintf("DROPS/results/CV2/modfits.MB.CV2.SE.%d-%d.rds", datasets[1], datasets[length(datasets)]))
}
